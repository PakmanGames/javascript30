...
const born1500 = inventors.filter(person => person.year >= 1500 && person.year < 1600);
console.table(born1500);

// Array.prototype.map()
// 2. Give us an array of the inventors first and last names
const firstAndLast = inventors.map(person => `${person.first} ${person.last}`);
console.table(firstAndLast);

// Array.prototype.sort()
// 3. Sort the inventors by birthdate, oldest to youngest
const sortedInventors = inventors.sort((person1, person2) => person1.year < person2.year ? -1:1);
console.table(sortedInventors);

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const totalLived = inventors.reduce((acc, current) => acc += (current.passed - current.year), 0);
...
const deFilter = boulevards.filter(boulevard => boulevard.includes("de"));
console.log(deFilter);
